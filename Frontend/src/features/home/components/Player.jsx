...
const Player = () => {
  ...
  const handleSongEnd = () => {
    ...
    if (nextIndex < song.length) {
      setCurrentIndex(nextIndex); // ✅ next song
    } else {
      // ✅ playlist khatam
      setIsPlaying(false);
      setCurrentTime(0);
  ...
  return (
    <main>
      <div className="player-container">
        <h3>Current Playlist : {currentSong.mood}</h3>
        <div className="player__playlist">
          {song.map((s, index) => (
            <div
              key={s._id}
              className={`playlist__item${index === currentIndex ? "active" : ""}`}
              onClick={() => setCurrentIndex(index)} // ✅ click se bhi change ho
            >
              <img src={s.posterUrl} alt={s.title} width={40} />
              <h4>{s.title}</h4>
            </div>
          ))}
        </div>
